...
const NodeList = () => {
  ...
  const generateShareLink = (node, sequenceNumber = null) => {
    const { protocol, node_password, uuid, host_config } = node;
    ...
    switch(protocol.toLowerCase()) {
      case 'shadowsocks': {
        ...
        return `ss://${CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse(ssConfig))}@${hosts}:${ports}?type=tcp#${displayName}`;
      }
      case 'vmess': {
        const vmessConfig = {
          "v": "2",
