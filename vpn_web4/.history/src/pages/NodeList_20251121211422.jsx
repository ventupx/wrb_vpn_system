...
const NodeList = () => {
  ...
  const generateShareLink = (node, index = 0) => {
    ...
    // 如果提供了索引号且大于0，则在国家名称后添加序号
    const countryWithExpiry = `${country || 'Unknown'}${formatExpiryDate(expiry_time)}${index > 0 ? `-${index}` : ''}`;
    
    switch(protocol.toLowerCase()) {
      case 'shadowsocks': {
        const ssConfig = `2022-blake3-aes-256-gcm:${node_password}:${node_password}`;
        return `ss://${CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse(ssConfig))}@${hosts}:${ports}?type=tcp#${countryWithExpiry}`;
      }
      case 'vmess': {
        const vmessConfig = {
          "v": "2",
          "ps": countryWithExpiry,
          "add": hosts,
          "port": String(ports),
          "id": uuid,
          "aid": "0",
      ...
      case 'vless':
        return `vless://${uuid}@${hosts}:${ports}?encryption=none&security=none&type=tcp#${countryWithExpiry}`;
      default:
        return '';
    }
  };
...
// 获取国家列表
  ...
  const searchNodes = async () => {
    ...
    try {
      ...
      const response = await request.get(`/customers/nodes/?page=${pagination.current}&page_size=${pagination.pageSize}&country=${selectedCountry?selectedCountry:''}&start_date=${dateRange?dateRange[0].format('YYYY-MM-DD'):''}&end_date=${dateRange?dateRange[1].format('YYYY-MM-DD'):''}&order_no=${searchText?searchText:''}`);
      if (response.results) {
  ...
  const handleExportSelected = () => {
    ...
    const selectedNodes = nodes.filter(node => selectedRowKeys.includes(node.key));
    
    // 处理相同订单号的节点
    const tradeNoMap = {};
    const shareLinks = selectedNodes.map(node => {
      // 如果节点有订单号，检查是否已经存在相同订单号的节点
      if (node.trade_no) {
        if (!tradeNoMap[node.trade_no]) {
          tradeNoMap[node.trade_no] = 0;
        }
        // 增加该订单号的计数
        tradeNoMap[node.trade_no]++;
        // 如果是该订单号的第一个节点，索引为0，否则为计数值-1
        const index = tradeNoMap[node.trade_no] > 1 ? tradeNoMap[node.trade_no] - 1 : 0;
        return generateShareLink(node, index);
      }
      return generateShareLink(node, 0);
    }).filter(link => link);
  ...
  const rowSelection = {
    ...
    getCheckboxProps: (record) => ({
      disabled: !['shadowsocks', 'vmess', 'vless'].includes(record.protocol.toLowerCase()),
    }),
  };
      ...
      <Card
        ...
        title={
          <div className="flex items-center">
            ...
            <span className="text-xl font-bold">服务器列表</span>
          </div>
        }
        className="shadow-md"
      >
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              {currentNode.udp_host ? (
                ...
                <>
                  <p><strong>默认IP地址：</strong> {currentNode.host}</p>
